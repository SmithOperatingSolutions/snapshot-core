...
| listing vcs's maps and their largest values (#26) | Merge wrote whatever conflict a model reported, while the conflict record's decoder refuses more than 10,000 model conflicts at a path, a location over 4,096 bytes or a reason over 1,024: a model over any limit merged cleanly and left a merge state whose conflicts could be neither listed nor resolved | Merge checks every conflict against the decoder's own constants before writing anything: `ErrConflictTooLarge`, the branch unchanged (`TestRegression_SC26_AConflictTooLargeToRecordRefusesTheMerge`) |
