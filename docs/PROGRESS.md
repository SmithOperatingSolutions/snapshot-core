...
| --- | --- | --- |
| `auth`, `cdc`, `wire`, `model`, `object` | 100% | 90% |
| `gc` | 98.7% | 90% |
| `model/tree` | 98.5% | 90% |
...
- [x] 50 concurrent root swappers on `s3` (MinIO in CI in the spec's words; SeaweedFS since 2026-09-24, D11): exactly one wins per round, none lost (`ConcurrentSwappersOneWinnerPerRound` with 50 swappers in `TestContractAgainstTheInProcessServer` and `TestContractAgainstRealS3`)
...
| the memory figures after #10 (#14) | The collection peak read 54 MiB where #6 had measured 31: not the 8 MiB index-object batches (1 MiB batches measure the same) and not the sampler (collecting twice changes nothing); a heap profile at the peak is 32 MiB of `pack.NewWriter`, the repack writer's buffer presized at the pack size since #10 for a repack of a few KiB of working-set nodes, and 20 MiB of zstd encoder that was always there | The repack's writer is sized to what is left to copy (`TestARepackAllocatesForWhatItCopies`, `TestAWriterSizedToWhatItWillHoldGrowsToAPack`): the peak is 30 MiB again, and the guard's bound came down from 96 to 48 MiB |
| the S3 tier (D11) | MinIO ended its public distribution: the pinned image vanished from quay.io between two CI runs on 2026-09-24, Docker Hub's `minio/minio` is 404 and dl.min.io answers 410; the tier failed on every PR. SeaweedFS passes the probe, the root swap and the rest of the contract, but lists a directory's children right after the directory, so the byte-order paging test failed on the contract's contrived names | The tier runs SeaweedFS pinned by digest; the port promises a total order kept across pages and `after` (byte order on every backend of ours), and the contract takes `ListOrder`, judged by `orderVerdict`/`prefixVerdict` (`TestListingVerdicts`); the real provider weekly keeps byte order |
| the weekly fuzz job (#13) | `FuzzOpenKeyFile` hung the fuzzing process on a key file inside the Argon2 ceiling (4 GiB, 64 passes), which OpenKeyFile derived as asked; the input stayed on the runner and was lost | The ceiling is a gibibyte and ten passes, refused before deriving (`TestKeyFileParameterBounds`; at the ceiling a file opens, `TestSlowAKeyFileAtTheCeilingOpens`); the job uploads `testdata/fuzz` on failure |
| the first models built outside the core (2026-09-24) | The model contract checked the merge identities and never a conflict, so a model that took ours on every collision passed; each map-shaped model copied about 120 lines of plumbing from `model/tree`; `core/internal/wire` was out of reach, so each copied a bounded reader; the helper's resolver gave one reason per key and no error, so a document's two colliding fields were one conflict and an undecodable record a conflict for a person | The contract requires a `Collide` and a conflict with a reason (`TestCollisionVerdict`); `model/mapobject` holds the plumbing, tree on it (`TestMergePerKey` and the rest); `core/wire` is public; `MergeWith` takes a `Decider` whose conflicts are located below the key and whose error aborts (`TestADeciderLocatesConflictsBelowTheKey`, `TestADecidersErrorAbortsTheMerge`) |
| a consumer's protected branches (2026-09-24) | Committing, merging, resolving and abandoning a merge all asked for `Write` on the branch, so no authorizer could let a principal merge into main and commit the merge while refusing direct writes to it | `auth.Commit` and `auth.Merge`; commit asks for Commit, the one-publish commit for Write and Commit, merge, resolve and abort for Merge; per-path checks stay Write (`TestCommittingIsItsOwnAction`, `TestMergingIsItsOwnAction`, `TestAProtectedBranchTakesMergesNotDirectWrites`) |
| a consumer's read-modify-write load under contention (2026-09-25) | A writer that lost the root swap finished, uploaded and fsynced its pack and index object before it learned, about 11 ms each, holding the store's commit lock; the version graph retried at once, up to 1,000 times | The publish reads the root first and a loser writes nothing (`TestALoserLearnsBeforeItWrites`); the race after the check still loses (`TestARootThatMovesDuringTheUploadStillLosesTheSwap`, `TestARootThatMovesAndReturnsDuringTheUploadStillPublishes`); lost swaps back off (`TestLostSwapsBackOffBetweenAttempts`, `TestAResolveThatLostItsWorkingSetBacksOff`, `TestAPauseEndsWithItsContext`). At 16 sessions: 2.8 to 16.3 tx/s on disk, 9.4 to 20.4 in memory (provisional, measured beside other work) |
| an audit after a fuzz run OOM-killed the host (#23) | A stream index node may name one chunk many times (valid: identical content cuts into identical chunks), so a Ref's size can be thousands of times what it stores, and `stream.ReadAll` read whatever it claimed; every map-shaped model read long values through it (Get, iteration, Diff, Walk). ReadAll also pre-grew its buffer by the claimed size, and a read re-read the whole path from the root for every data chunk, so small chunks under large nodes cost the index times the length. The format needed no change: entries carry sizes, checked as read | `ReadAll` takes a limit and refuses a longer stream unread (`TestRegression_SC23_ReadAllRefusesAStreamOverItsLimit`, `TestRegression_SC23_ReadAllGrowsWithWhatItReads`); a read descends from the checked nodes it holds (`TestRegression_SC23_ASequentialReadReadsEachNodeOncePerPlace`, `TestEveryByteReadAloneIsTheSourcesByte`); `prolly.Config.MaxValue`, default 64 MiB, bounds what a map reads and takes (`TestRegression_SC23_AMapReadsNoValueOverItsLimit`, `TestRegression_SC23_AnEditorTakesNoValueOverItsLimit`, `TestRegression_SC23_NoLimitNamedIsTheDefault`); the tree reads no record past `EntrySize` and a namespace none past `RefSize` (`TestRegression_SC23_ATreeReadsNoRecordLongerThanAnEntry`, `TestRegression_SC23_ANamespaceReadsNoValueLongerThanARef`); `FuzzStreamStructure` builds multi-chunk trees with repeated children. Walk's leaf visits were not an amplification: a walk that goes into each node once calls visit once per entry of the nodes it enters, which the fuzz target holds |
| the same audit (#24) | `model/tree` sized its result map by the prolly root's claimed count, checked only as iteration reaches each child (11.6 MB for a two-entry tree claiming 65,537); `prolly.decodeNode` presized entries from a node's header count (2.2 MB for a 64 KiB node), and `pack.decodeIndex` from an index's (3 MiB for nine bytes) | Nothing sized by a claim: `TestRegression_SC24_ReadingATreeDoesNotSizeByItsClaimedCount`, `TestRegression_SC24_ANodeIsNotSizedByItsClaimedCount`, `TestRegression_SC24_AnIndexIsNotSizedByItsClaimedCount` |
...
- [x] #6 The index in memory: a `dedup` table on disk (`TestATableAnswersEveryRecordItWasBuiltFrom`, `TestTheFirstRecordOfAKeyWins`, `TestATableHoldsAlmostNothingInMemory`, `TestATableThatDoesNotDecodeIsRefused`, `FuzzOpenTable`); a store spills its index past a bound (`TestAStoreSpillsItsIndexToDisk`, `TestASpilledIndexIsRebuiltWhenGCMovesIt`, `TestASpilledIndexNeedsItsDirectory`, `TestContractOverASpilledIndex`, `TestPacksInServiceComeFirstInTheIndex`); GC marks and indexes on disk (`TestGCNeedsItsWorkDirectory`); with 64 Ki in memory, a million chunks open in 21 KiB and collect in a 30 MiB peak, two million in 25 KiB and 34 MiB (`TestSlowMemoryPerChunkOn1MChunks`; 54 MiB for a while after #10 presized the repack writer's buffer at the pack size, #14).
- [x] #4 The real provider, weekly: `TestContractAgainstRealS3` and `TestARepositoryRunsOnTheRealProvider` ran green against the real provider in the weekly run of 2026-09-23 (the first run had found the endpoint given as a bare host: accepted as HTTPS). The S3 tier (SeaweedFS) runs the e2e package on every push.
...
10. ✅ **Every input-sized allocation bounded** (#22 to #27, v0.2.0): the
    fuzz step caps its workers and their memory; `stream.ReadAll` takes a
    limit and the reader reads each node once per place; `prolly.Config.MaxValue`
    bounds map values, with tight limits for trees, namespaces and the
    version graph's own maps; nothing is sized from a claimed count; the
    key-file fuzzer never derives above its budget; index frames tie stored
    to raw length and lie where their pack holds them; a chunk read stops at
    its range; `Merge` refuses a conflict too large to record. Each has a
    `TestRegression_SC<n>_...` whose red asserted a small budget.
11. ✅ **What a consumer's load test found** (v0.2.0): a publishing writer
    that lost the race learns it before uploading, and lost swaps back off
    (`TestALoserLearnsBeforeItWrites`, `TestLostSwapsBackOffBetweenAttempts`);
    a refresh of an unmoved root opens no manifest, publishes compact small
    index objects, and a pending pack starts small
    (`TestARefreshOfAnUnmovedRootOpensNoManifest`,
    `TestPublishesCompactSmallIndexObjects`, `TestSmallCommitsAllocateForSmallPacks`);
...
16. ✅ **Journaled commits** (#34, D16, off by default pending the owner):
